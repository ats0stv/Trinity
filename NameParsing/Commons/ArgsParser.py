...
class ArgsParser:
    ...
    def parseArguments(self):
        ...
        parser.add_argument(
            "--outputFile", "-o", help="If a file path is present, it writes the output to the file, else on console", required=False)
        parser.add_argument(
            '--pretty', '-p', help="Pretty print the JSON/XML output", action='store_true', required=False)
        outputTypeGroup = parser.add_mutually_exclusive_group(required=True)
        outputTypeGroup.add_argument(
            '--xml', help="Render output as XML", action='store_true')
        outputTypeGroup.add_argument(
            '--json', help="Render output as JSON", action='store_true')
